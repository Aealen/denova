...
for (const kind of ['writing', 'general'] as const) {
  for (const theme of ['dark', 'light']) {
    test(`${kind} composer saves Codex model and effort in ${theme}`, async ({ page, request }) => {
      ...
      const seeded = await request.patch('/api/settings', { data: { layer: 'user', base_revision: current.revisions.user,
        changes: { theme, agent_runtimes: { [role]: { selected: 'codex', codex: model } } } } })
      expect(seeded.ok(), await seeded.text()).toBe(true)
      let projectId: string
      let sessionId: string
      if (kind === 'writing') {
        projectId = (await createAndOpenBook(request, `Codex composer ${theme}`)).projectId
        const response = await request.post('/api/sessions', { data: { title: 'Codex selection' } })
        expect(response.ok(), await response.text()).toBe(true)
        sessionId = (await response.json()).id
        expect((await request.post('/api/sessions/switch', { data: { id: sessionId } })).ok()).toBe(true)
      } else {
        const directory = await mkdtemp(path.resolve('test-results', 'runtime', 'codex-composer-'))
        projectId = (await registerAgentChatProject(request, directory)).id
        sessionId = (await createAgentChatSession(request, projectId, 'Codex selection')).id
      }
      await page.route('**/api/agent-runtimes/codex/models', route => route.fulfill({ json: { items: [
      ...
      const readSelection = async () => (await (await request.get(configPath)).json()).runtime.codex
      const openComposer = async () => {
        if (kind === 'writing') await openWritingAgent(page)
        else { await openAgentChatWorkbench(page); await openAgentChatSession(page, projectId, 'Codex selection') }
      }
      await page.goto('/')
      await openComposer()
      ...
      for (const width of [1440, 390]) {
        ...
        await page.screenshot({ path: test.info().outputPath(`codex-composer-${width}.png`) })
      ...
      expect(saved.user.agent_runtimes[role].codex).toEqual(model)
    })
  }
}
