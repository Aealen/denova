...
func (operation *Operation) invoke(ctx context.Context, call ToolCall, tool preparedTool) (ToolResult, error) {
	...
	operation.send(agentrun.Event{Type: "tool_result", Data: map[string]any{"id": executionID, "name": call.Name, "content": processed.DisplayContent, "status": string(processed.Status), "run_id": operation.id, "tool_presentation": tool.definition.Descriptor.Presentation}})
