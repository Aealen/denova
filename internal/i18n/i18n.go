...
var catalogZH = map[string]string{
	...
	"agentRuntime.notReady":              "Agent 引擎尚未就绪，请检查连接；如需登录，请在运行 Denova 的电脑上执行 codex login。",
...
var catalogEN = map[string]string{
	...
	"agentRuntime.notReady":              "The Agent runtime is not ready. Check the connection; if sign-in is required, run codex login on the computer running Denova.",
